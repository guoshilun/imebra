            ...
            try {
                for (size_t buffer(0);; ++buffer) {
                    try {
                        ...
                        size_t  sourceSize = sourceHandler.getSize();
                        destHandler.setSize(sourceSize);
                        std::wcout << L"SourceSize :" <<  sourceSize << std::endl;
                        for (size_t item(0); item != sourceSize; ++item) {
                            tagVR_t vr = sourceHandler.getDataType();

                            std::wstring itemValue = sourceHandler.getUnicodeString(item);
                            std::wcout << L"==========" << std::endl;
                            std::wcout << L"ItemValue :" << itemValue << std::endl;
                            if (vr == tagVR_t::UI) {
                                std::wcout << L"UI" << std::endl;
                            }
                            destHandler.setUnicodeString(item, itemValue);
                        }
                        std::wcout << "ReadWring Over!" << std::endl;
                    }
                }
            }
            catch (const MissingDataElementError &) {
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        if (argc == 4) {
            ...
            if (transferSyntaxValue >= 0 &&
                (size_t) transferSyntaxValue < sizeof(transferSyntaxAllowedValues) / sizeof(char *)) {
