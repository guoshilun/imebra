...
namespace imebra {
    ...
    charsetDictionary::charsetDictionary() {
        IMEBRA_FUNCTION_START() ;
            ...
            registerCharset("ISO 2022 IR 100",
                            "\x1b\x2d\x41", "ISO-IR-100", "ISO-8859-1", 28591,
                            "\x1b\x28\x42", "ISO-IR-6", "US-ASCII", 20127,
                            false);

            registerCharset("ISO 2022 IR 101",
                            "\x1b\x2d\x42", "ISO-IR-101", "ISO-8859-2", 28592,
                            "\x1b\x28\x42", "ISO-IR-6", "US-ASCII", 20127,
                            false);
            registerCharset("ISO 2022 IR 109",
                            "\x1b\x2d\x43", "ISO-IR-109", "ISO-8859-3", 28593,
                            "\x1b\x28\x42", "ISO-IR-6", "US-ASCII", 20127,
                            false);
            registerCharset("ISO 2022 IR 110",
                            "\x1b\x2d\x44", "ISO-IR-110", "ISO-8859-4", 28594,
                            "\x1b\x28\x42", "ISO-IR-6", "US-ASCII", 20127,
                            false);
            registerCharset("ISO 2022 IR 144",
                            "\x1b\x2d\x4c", "ISO-IR-144", "ISO-8859-5", 28595,
                            "\x1b\x28\x42", "ISO-IR-6", "US-ASCII", 20127,
                            false);
            registerCharset("ISO 2022 IR 127",
                            "\x1b\x2d\x47", "ISO-IR-127", "ISO-8859-6", 28596,
                            "\x1b\x28\x42", "ISO-IR-6", "US-ASCII", 20127,
                            false);
            registerCharset("ISO 2022 IR 126",
                            "\x1b\x2d\x46", "ISO-IR-126", "ISO-8859-7", 28597,
                            "\x1b\x28\x42", "ISO-IR-6", "US-ASCII", 20127,
                            false);
            registerCharset("ISO 2022 IR 138",
                            "\x1b\x2d\x48", "ISO-IR-138", "ISO-8859-8", 1255,
                            "\x1b\x28\x42", "ISO-IR-6", "US-ASCII", 20127,
                            false);
            registerCharset("ISO 2022 IR 148",
                            "\x1b\x2d\x4d", "ISO-IR-148", "ISO-8859-9", 28599,
                            "\x1b\x28\x42", "ISO-IR-6", "US-ASCII", 20127,
                            false);
            registerCharset("ISO 2022 IR 149",
            ...
            registerCharset("ISO 2022 IR 166",
                            "\x1b\x2d\x54", "ISO-IR-166", "TIS-620", 874,
                            "\x1b\x28\x42", "ISO-IR-6", "US-ASCII", 20127,
                            false);
            ...
            registerCharset("ISO 2022 IR 58", "GB18030", "GB18030", 54936, true);

        IMEBRA_FUNCTION_END();
    }

    const charsetInformation &
    charsetDictionary::getCharsetInformation(const std::string &dicomName, size_t variant) const {
        IMEBRA_FUNCTION_START() ;
            ...
            if (variant > 0) {
                std::ostringstream makeName;
                makeName << normalizedName << "/" << variant;
                normalizedName = makeName.str();
            }
            auto findInfo(m_dictionary.find(normalizedName));
            if (findInfo == m_dictionary.end()) {
                IMEBRA_THROW(CharsetConversionNoTableError,
                             "Charset table " << dicomName << " not found in the charset dictionary");
            }
    ...
    void charsetDictionary::registerCharset(const std::string &dicomName, const std::string &isoName,
                                            const std::string &javaName, const unsigned long windowsPage,
        ...
        IMEBRA_FUNCTION_START() ;
            ...
            m_dictionary.insert(std::pair<std::string, charsetInformation>(
                    ...
                    charsetInformation(dicomName,
                                       "", isoName, javaName, windowsPage,
                                       bZeroFlag)));

            m_dictionary.insert(std::pair<std::string, charsetInformation>(
                    charsetConversionBase::normalizeIsoCharset(dicomName) + "/1",
                    charsetInformation(dicomName,
                                       "", isoName, javaName, windowsPage,
                                       bZeroFlag)));
    ...
    void charsetDictionary::registerCharset(const std::string &dicomName,
                                            const std::string &escapeSequence0, const std::string &isoName0,
                                            const std::string &javaName0, const unsigned long windowsPage0,
                                            const std::string &escapeSequence1, const std::string &isoName1,
                                            const std::string &javaName1, const unsigned long windowsPage1,
                                            const bool bZeroFlag) {
        IMEBRA_FUNCTION_START() ;

            std::string normalizedDicomName(charsetConversionBase::normalizeIsoCharset(dicomName));

            m_dictionary.insert(std::pair<std::string, charsetInformation>(
                    normalizedDicomName,
                    charsetInformation(dicomName,
                                       escapeSequence0, isoName0, javaName0, windowsPage0,
                                       bZeroFlag)));

            if (!escapeSequence0.empty()) {
                m_escapeSequences[escapeSequence0] = &(m_dictionary.at(normalizedDicomName));
            }

            if (!escapeSequence1.empty() && escapeSequence0 != escapeSequence1) {
                std::string g0DicomName = normalizedDicomName + "/1";
                m_dictionary.insert(std::pair<std::string, charsetInformation>(
                        g0DicomName,
                        charsetInformation(dicomName,
                                           escapeSequence1, isoName1, javaName1, windowsPage1,
                                           bZeroFlag)));

                m_escapeSequences[escapeSequence1] = &(m_dictionary.at(g0DicomName));
            }

        IMEBRA_FUNCTION_END();
    }

    charsetInformation::charsetInformation(const std::string &dicomName, const std::string &escapeSequence,
                                           const std::string &isoRegistration, const std::string &javaRegistration,
                                           unsigned long codePage, bool bZeroFlag) :
...
///////////////////////////////////////////////////////////
    ...
    const charsetDictionary &charsetConversionBase::getDictionary() {
        IMEBRA_FUNCTION_START() ;
