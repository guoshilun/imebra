...
///////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////
            void dicomStreamCodec::writeStream(std::shared_ptr<streamWriter> pStream,
                                               std::shared_ptr<dataSet> pDataSet) const {
                IMEBRA_FUNCTION_START() ;
                    ...
                    // Adjust the flags
                    ///////////////////////////////////////////////////////////
                    bool bExplicitDataType = (transferSyntax !=
                                              "1.2.840.10008.1.2");        // Implicit VR little endian

                    // Explicit VR big endian
                    ///////////////////////////////////////////////////////////
                    streamController::tByteOrdering endianType = (transferSyntax == "1.2.840.10008.1.2.2")
                                                                 ? streamController::tByteOrdering::highByteEndian
                                                                 : streamController::tByteOrdering::lowByteEndian;
...
///////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////
            void dicomStreamCodec::buildStream(std::shared_ptr<streamWriter> pStream,
                                               std::shared_ptr<const dataSet> pDataSet, bool bExplicitDataType,
                                               streamController::tByteOrdering endianType, streamType_t streamType) {
                IMEBRA_FUNCTION_START() ;

                    dataSet::tGroupsIds groups = pDataSet->getGroups();

                    for (dataSet::tGroupsIds::const_iterator scanGroups(groups.begin()), endGroups(groups.end());
                         scanGroups != endGroups; ++scanGroups) {
                        ...
                        for (size_t scanGroupsNumber(0); scanGroupsNumber != numGroups; ++scanGroupsNumber) {
                            ...
                            if (*scanGroups == 0x0002) {
                                ...
                                if (streamType == streamType_t::mediaStorage) {
                                    ...
                                    std::shared_ptr<data> metaInformationTag(
                                            std::make_shared<data>(tagVR_t::OB, charsets));
                                    {
                                        std::shared_ptr<handlers::writingDataHandler> handler(
                                                metaInformationTag->getWritingDataHandler(0));
                                    ...
                                    std::shared_ptr<data> implementationClassUidTag(
                                            std::make_shared<data>(tagVR_t::UI, charsets));
                                    {
                                        std::shared_ptr<handlers::writingDataHandler> handler(
                                                implementationClassUidTag->getWritingDataHandler(0));
                                    ...
                                    std::shared_ptr<data> implementationNameTag(
                                            std::make_shared<data>(tagVR_t::SH, charsets));
                                    {
                                        std::shared_ptr<handlers::writingDataHandler> handler(
                                                implementationNameTag->getWritingDataHandler(0));
...
///////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////
            void dicomStreamCodec::writeGroup(std::shared_ptr<streamWriter> pDestStream, const dataSet::tTags &tags,
                                              std::uint16_t groupId, bool bExplicitDataType,
                ...
                IMEBRA_FUNCTION_START() ;
                    ...
                    if (groupId == 0 || groupId == 2) {
                        ...
                        if (bExplicitDataType) {
                            ...
                            const std::uint16_t tagLengthWord = pDestStream->adjustEndian(std::uint16_t(4u),
                                                                                          endianType);;
                            pDestStream->write(reinterpret_cast<const std::uint8_t *>(&tagLengthWord), 2u);
                        } else {
                            const std::uint32_t tagLengthDword = pDestStream->adjustEndian(std::uint32_t(4u),
                                                                                           endianType);
                    ...
                    // Write all the tags
                    ///////////////////////////////////////////////////////////
                    for (dataSet::tTags::const_iterator scanTags(tags.begin()), endTags(tags.end());
...
///////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////
            void dicomStreamCodec::writeTag(std::shared_ptr<streamWriter> pDestStream, std::shared_ptr<data> pData,
                                            std::uint16_t tagId, bool bExplicitDataType,
                                            streamController::tByteOrdering endianType) {
                IMEBRA_FUNCTION_START() ;
                    ...
                    //  endian)
                    ///////////////////////////////////////////////////////////
                    const std::uint16_t sequenceItemGroup = streamController::adjustEndian(std::uint16_t(0xfffe),
                                                                                           endianType);
                    const std::uint16_t sequenceItemDelimiter = streamController::adjustEndian(std::uint16_t(0xe000),
                                                                                               endianType);
                    const std::uint16_t sequenceTagEnd = streamController::adjustEndian(std::uint16_t(0xe0dd),
                                                                                        endianType);
                    ...
                    if (bExplicitDataType) {
                        std::string dataTypeString(
                                dicomDictionary::getDicomDictionary()->enumDataTypeToString(dataType));
                        ...
                        if (dicomDictionary::getDicomDictionary()->getLongLength(dataType)) {
                            ...
                            if (bSequence) {
                                IMEBRA_THROW(InvalidSequenceItemError,
                                             "Sequences cannot be used with dataType " << dataTypeString);
                            }
                    ...
                    for (std::uint32_t scanBuffers = 0;; ++scanBuffers) {
                        if (pData->bufferExists(scanBuffers)) {
                            ...
                            if (pBuffer->hasExternalStream() &&
                                (wordSize < 2u || endianType == pBuffer->getEndianType())) {
                                ...
                                try {
                                    ...
                                    if (writtenSize != writeSize - 1) {
                                        IMEBRA_THROW(StreamEOFError,
                                                     "An external stream containing a tag value cannot be read");
                                    }
                                ...
                                ///////////////////////////////////////////////////////////
                                std::shared_ptr<handlers::readingDataHandlerRaw> pDataHandlerRaw = pData->getReadingDataHandlerRaw(
                                        scanBuffers);

                                if (wordSize > 1) {
                                    std::vector<std::uint8_t> tempBuffer(writeSize);
                                    ::memcpy(tempBuffer.data(), pDataHandlerRaw->getMemoryBuffer(),
                                             pDataHandlerRaw->getSize());
                                    if (writeSize != bufferSize) {
                                        tempBuffer[bufferSize] = pData->getPaddingByte();
                                    }
                                    if (pBuffer->getEndianType() != endianType) {
                                        streamController::reverseEndian(tempBuffer.data(), wordSize,
                                                                        writeSize / wordSize);
                                    }
                                    ...
                                    pDestStream->write(
                                            reinterpret_cast<const std::uint8_t *>(pDataHandlerRaw->getMemoryBuffer()),
                                            bufferSize);
...
///////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////
            std::uint32_t dicomStreamCodec::getTagLength(const std::shared_ptr<data> &pData, bool bExplicitDataType,
                                                         std::uint32_t *pHeaderLength, bool *pbSequence) {
                IMEBRA_FUNCTION_START() ;
...
///////////////////////////////////////////////////////////
            std::uint32_t
            dicomStreamCodec::getDataSetLength(std::shared_ptr<dataSet> pDataSet, bool bExplicitDataType) {
                IMEBRA_FUNCTION_START() ;
                    ...
                    buildStream(pWriter, pDataSet, bExplicitDataType, streamController::tByteOrdering::lowByteEndian,
                                streamType_t::normal);
...
///////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////
            void dicomStreamCodec::readStream(std::shared_ptr<streamReader> pStream, std::shared_ptr<dataSet> pDataSet,
                                              std::uint32_t maxSizeBufferLoad /* = 0xffffffff */) const {
                IMEBRA_FUNCTION_START() ;
                    ...
                    while (!pStream->endReached() && (*pReadSubItemLength < subItemLength)) {
                        ...
                        if (tagId != 0x0002 && bCheckTransferSyntax) {
                            ...
                            std::string transferSyntax = pDataSet->getString(
                                    ...
                                    0,
                                    0,
                                    endianType == streamController::tByteOrdering::lowByteEndian ? "1.2.840.10008.1.2.1"
                                                                                                 : "1.2.840.10008.1.2.2");
                        ...
                        if (bExplicitDataType && tagId != 0xfffe) {
                            ...
                            try {
                                ...
                                if (dicomDictionary::getDicomDictionary()->getLongLength(tagType)) {
                                    ...
                                    pStream->adjustEndian((std::uint8_t *) &tagLengthDWord, sizeof(tagLengthDWord),
                                                          endianType);
                                ...
                                if (endianType == streamController::tByteOrdering::lowByteEndian)
                                    tagLengthDWord = (((std::uint32_t) tagLengthWord) << 16) |
                                                     ((std::uint32_t) tagTypeString[0]) |
                                                     (((std::uint32_t) tagTypeString[1]) << 8);
                                else
                                    tagLengthDWord =
                                            (std::uint32_t) tagLengthWord | (((std::uint32_t) tagTypeString[0]) << 24) |
                                            (((std::uint32_t) tagTypeString[1]) << 16);
                            }
                        ...
                        if (tagLengthDWord != 0xffffffff && tagType != tagVR_t::SQ) {
                            (*pReadSubItemLength) += readTag(pStream, pDataSet, tagLengthDWord, tagId, order, tagSubId,
                                                             tagType, endianType, wordSize, 0, maxSizeBufferLoad);
                        ...
                        while (tagLengthDWord && !pStream->endReached()) {
                            ...
                            pStream->adjustEndian((std::uint8_t *) &sequenceItemLength, sizeof(sequenceItemLength),
                                                  endianType);
                            ...
                            if ((sequenceItemLength == 0xffffffff) || tagType == tagVR_t::SQ) {
                                ...
                                parseStream(pStream, sequenceDataSet, bExplicitDataType, endianType, maxSizeBufferLoad,
                                            sequenceItemLength, &effectiveLength, depth + 1);
                            ...
                            ///////////////////////////////////////////////////////////
                            sequenceItemLength = readTag(pStream, pDataSet, sequenceItemLength, tagId, order, tagSubId,
                                                         tagType, endianType, wordSize, bufferId++, maxSizeBufferLoad);
...
///////////////////////////////////////////////////////////
            std::uint32_t dicomStreamCodec::readTag(
                ...
                IMEBRA_FUNCTION_START() ;
                    ...
                    ///////////////////////////////////////////////////////////
                    std::shared_ptr<handlers::writingDataHandlerRaw> handler(
                            pDataSet->getWritingDataHandlerRaw(tagId, order, tagSubId, bufferId, tagType));
                    ...
                    } else // Read in small chunks
                    {
                        ...
                        while (remainingBytes != 0) {
                            ...
                            ///////////////////////////////////////////////////////////
                            std::uint32_t thisBufferSize(
                                    (remainingBytes > smallBuffersSize) ? smallBuffersSize : remainingBytes);
                        ...
                        for (smallBuffersIterator = buffers.begin();
                             smallBuffersIterator != buffers.end(); ++smallBuffersIterator) {
                            std::uint32_t copySize = (remainingBytes > smallBuffersSize) ? smallBuffersSize
                                                                                         : remainingBytes;
                    ...
                    ///////////////////////////////////////////////////////////
                    if (wordSize != 0) {
                        pStream->adjustEndian(handler->getMemoryBuffer(), wordSize, endianType,
                                              tagLengthDWord / wordSize);
                    }
